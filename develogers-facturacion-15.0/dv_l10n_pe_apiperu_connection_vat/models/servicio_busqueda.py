...
def get_ruc_apiperu(token, ruc):
	...
	if datos_ruc.status_code == 200:
		...
		_logger.info(datos_ruc['data'])
		direccion = datos_ruc['data']['direccion_completa'] if 'direccion_completa' in datos_ruc['data'] and  datos_ruc['data']['direccion_completa'] else ''
		datos = {
			'error': False, 
			'message': 'ok',
