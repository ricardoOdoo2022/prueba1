...
class PeruSunatCpe(models.Model):
    ...
    def send_cpe(self):
        ...
        _logging.info("response_status: %s" % response_status)
        _logging.info("response: %s" % response)
        _logging.info("response_data: %s" % response_data)
        if response_status:
            res = "verify"
            if self.type == "sync":
            ...
            res = "send"
            _logging.info("Enviando CPE1: %s" % self.name)
            _logging.info("response: %s" % response)
    ...
    def get_sunat_ticket_status(self):
        ...
        if response_status:
            ...
            res = "send"
            _logging.info("Enviando CPE2: %s" % self.name)
            _logging.info("response: %s" % response)
