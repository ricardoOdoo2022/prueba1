...
class Document(object):
    ...
    def send(self):
        if self._type == 'sync':
            self._zip_file = b64encode(self.in_memory_data.getvalue())
            _logging.info('_client2')
            _logging.info(self._client._username)
            _logging.info(self._client._password)
    ...
    def process(self, document_name, type, xml, client):
        ...
        self._client = client
        _logging.info('_xml: %s' % self._xml)
        _logging.info('_type: %s' % self._type)
        _logging.info('_document_name: %s' % self._document_name)
        _logging.info('_client:', self._client)
        self.prepare_zip()
        self.send()
    ...
    def get_response(file, name):
        zf = zipfile.ZipFile(BytesIO(b64decode(file)))
...
class Client(object):
    ...
    def __init__(self, ruc, username, password, url, debug=False, type=None, server=None):
        ...
        self._connect()
        _logging.info("INIT CLIENT")

    def _connect(self):
        if not self._type:
            _logging.info('_type1: %s' % self._type)
            ...
            _logging.info('_type2: %s' % self._type)
            #self._client = SoapClient(location=(self._location), action=(self._soapaction), namespace=(self._namespace), username=(self._username), password=(self._password), key_file=(self._key_file), cert=(self._cert_file))
            self._client = SoapClient(location=(self._location), action=(self._soapaction), namespace=(self._namespace))
            _logging.info('ABC1')
            _logging.info('_client:', self._client)
            _logging.info('_username: %s' % self._username)
            _logging.info('_password: %s' % self._password)
            _logging.info('_url: %s' % self._url)
            #self._client.username =
            #self._client.password =
            #self._client.key_file =
            #self._client.cert = ???? PORQUE SE ELIMINA????
            #self._client['wsse:Security'] = {'wsse:UsernameToken': {'wsse:Username':self._username,  'wsse:Password':self._password}}

    def _call_ws(self, xml):
        _logging.info('_method: %s' % self._method)
        _logging.info('_client:', self._client)
        _logging.info('_username: %s' % self._username)
        _logging.info('_password: %s' % self._password)
        _logging.info('_url: %s' % self._url)
        xml_response = self._client.send(self._method, xml.encode('utf-8'))
        _logging.info('xml_response: %s' % xml_response)
        vals = {}
        ...
        if root.find('.//faultstring') is not None:
            if self._type == 'statusCdr':
                ...
                state = False
        _logging.info('State: %s' % state)
        _logging.info('Vals: %s' % vals)
        return (state, vals)
    ...
    def _call_service(self, name, params):
        if not self._type:
            ...
            _logging.info('Calling service %s' % name)
            _logging.info('Params: %s' % params)
            _logging.info('Service: %s' % service)
            return (
                True, service(**params))
            #except SoapFault as ex:
            #    _logging.info('Error1: %s' % ex)
            #    _logging.info('Faultcode: %s' % ex.faultcode)
            #    _logging.info('Faultstring: %s' % ex.faultstring)
        ...
        _logging.info('XML: %s' % xml)
        return self._call_ws(xml)
        #except Exception as e:
        #	_logging.info('Error2: %s' % e)
        #	return (False, {})
...
def send_sunat_cpe(client, document):
    ...
    _logging.info('Document: %s' % document)
    _logging.info('Client: %s' % client)
