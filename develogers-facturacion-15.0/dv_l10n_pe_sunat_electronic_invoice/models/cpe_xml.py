...
class CPE:
    ...
    def _agregar_informacion_certificado(self, invoice_id):
        ...
        etree.SubElement(signature, (tag.text), nsmap={'cbc': tag.namespace}).text = invoice_id._name == 'solse.cpe' and invoice_id.name or invoice_id.name
    ...
    def _agregar_informacion_anticipos(self, invoice_id):
        try:
            for line in invoice_id.invoice_line_ids.mapped('sale_line_ids').mapped('order_id').mapped('invoice_ids').filtered(lambda inv: inv.pe_sunat_transaction in ('04', ) and inv.pe_invoice_code in ('01', '03') and inv.id != invoice_id.id and inv.state not in ('draft', 'cancel')):
    ...
    def _agregar_informacion_descuentos_globales(self, invoice_id):
        if invoice_id.pe_total_discount > 0.0:
            ...
            etree.SubElement(allowance_charge, (tag.text), listAgencyName='PE:SUNAT', listName='Cargo/descuento', listURI='urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo53', nsmap={'cbc': tag.namespace}).text = '00'
    ...
    def _agregar_informacion_de_impuestos(self, invoice_id):
        ...
        tax_amount_all = 0.0
        invoice_totals = json.loads(invoice_id.tax_totals_json)
        for amount_by_group_list in invoice_totals['groups_by_subtotal'].values():
            for group_tax in amount_by_group_list:
                tax = invoice_id.line_ids.mapped('tax_ids').filtered(lambda x: x.tax_group_id.id == group_tax['tax_group_id'])
                tax = tax and tax[0]
                if tax.l10n_pe_edi_tax_code != constantes.IMPUESTO['ICBPER']:
                    tax_amount_all += group_tax['tax_group_amount']
                    continue
    ...
    def _agregar_informacion_detraccion(self, invoice_id):
        if invoice_id.tiene_detraccion:
            ...
            tag = etree.QName(self._cbc, 'Amount')
            monto_detraccion = invoice_id.amount_total * invoice_id.porc_detraccion/100
            etree.SubElement(payment_terms, (tag.text), currencyID="PEN", nsmap={'cbc': tag.namespace}).text = str(round(monto_detraccion, 2))
            
    # Para la parte donde indica si la factura es de retencion
    def _agregar_informacion_retencion(self, invoice_id):
        if invoice_id.tiene_retencion:
            ...
            etree.SubElement(allowance_charge, (tag.text), nsmap={'cbc': tag.namespace}).text = 'false'
            tag = etree.QName(self._cbc, 'AllowanceChargeReasonCode')
            etree.SubElement(allowance_charge, (tag.text), nsmap={'cbc': tag.namespace}).text = '62'
            tag = etree.QName(self._cbc, 'MultiplierFactorNumeric')
            etree.SubElement(allowance_charge, (tag.text), nsmap={'cbc': tag.namespace}).text = str(round(invoice_id.porc_retencion/100,2))
            tag = etree.QName(self._cbc, 'Amount')
            monto_retencion = invoice_id.amount_total * invoice_id.porc_retencion/100
            etree.SubElement(allowance_charge, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = str(round(monto_retencion,2))
            tag = etree.QName(self._cbc, 'BaseAmount')
            #amount_total = invoice_id.amount_untaxed + (invoice_id.pe_total_discount - invoice_id.pe_total_discount_tax) - invoice_id.monto_retencion
            etree.SubElement(allowance_charge, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = str(round(invoice_id.amount_total,2))
    ...
    def _agregar_informacion_tipo_transaccion(self, invoice_id):
        if invoice_id.l10n_pe_edi_is_sale_credit:
            tag = etree.QName(self._cac, 'PaymentTerms')
            payment_terms = etree.SubElement((self._root), (tag.text), nsmap={'cac': tag.namespace})
            tag = etree.QName(self._cbc, 'ID')
            ...
            for due in invoice_id.l10n_pe_edi_dues_ids:
                tag = etree.QName(self._cac, 'PaymentTerms')
                ...
                tag = etree.QName(self._cbc, 'PaymentMeansID')
                etree.SubElement(payment_terms_cuota, (tag.text), nsmap={'cbc': tag.namespace}).text = f'Cuota{str(due.dues_number).zfill(3)}'
                tag = etree.QName(self._cbc, 'Amount')
                etree.SubElement(payment_terms_cuota, (tag.text), currencyID=(invoice_id.currency_id.name), nsmap={'cbc': tag.namespace}).text = str(round(due.amount, 2))
                tag = etree.QName(self._cbc, 'PaymentDueDate')
                etree.SubElement(payment_terms_cuota, (tag.text), nsmap={'cbc': tag.namespace}).text = str(due.paid_date)
        else:
            tag = etree.QName(self._cac, 'PaymentTerms')
            payment_terms = etree.SubElement((self._root), (tag.text), nsmap={'cac': tag.namespace})
            tag = etree.QName(self._cbc, 'ID')
    ...
    def getInvoice(self, invoice_id):
        ...
        self._agregar_informacion_retencion(invoice_id)
