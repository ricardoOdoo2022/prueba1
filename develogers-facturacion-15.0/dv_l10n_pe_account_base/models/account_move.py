...
class AccountMove(models.Model):
    ...
    # Dues con retencion
    @api.onchange('line_ids', 'invoice_payment_term_id', 'invoice_date_due', 'invoice_cash_rounding_id', 'invoice_vendor_bill_id', 'tiene_retencion', 'tiene_detraccion','porc_retencion', 'porc_detraccion')
    ...
    def _get_dues_ids(self):
        if self.l10n_pe_edi_is_sale_credit:
            ...
            for line in self.line_ids.filtered(lambda x: x.exclude_from_invoice_tab and x.date_maturity != False and x.debit > 0.0):
                number += 1
                if self.tiene_detraccion:
                    porc_detraccion = self.porc_detraccion
                else:
                    porc_detraccion = 0
                ...
                vals = {
                    ...
                    'amount': abs(line.amount_currency) * (1-(porc_retencion+porc_detraccion) / 100.0),
                }
    ...
    def _compute_detraccion_retencion(self):
        for reg in self:
            ...
            if reg.currency_id.id != reg.company_id.currency_id.id:
                ...
                reg.monto_neto_pagar = round(monto_neto_pagar,2)
                reg.monto_neto_pagar_base = round(monto_neto_pagar_base,2)
    ...
    def _validar_detraccion_retencion(self, forzar_retencion):
        ...
        if self.tiene_detraccion and (abs(self.amount_total_signed) < self.company_id.monto_detraccion or self.partner_id.doc_type != "6"):
            return datos_rpt
        ...
        if tiene_detraccion:
            ...
            monto_detraccion = round(monto_detraccion,2)
        ...
        monto_retencion = self.amount_total * float(self.invoice_date_currency_rate) * (self.porc_retencion / 100.0)
    ...
    def _get_starting_sequence(self):
        if self.usar_prefijo_personalizado and self.l10n_latam_document_type_id:
            ...
            if self.journal_id.code != 'INV':
                #middle_code = middle_code[:1] + self.journal_id.code[:2]
                middle_code = self.journal_id.code
